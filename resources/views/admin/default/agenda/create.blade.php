...
<div class="card">
    <div class="card-body">
        ...
        <div class="col-xl-12 col-md-12 col-sm-12">
            <form action="{{ route('admin.agenda.store') }}" id="user-form" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-group ">
                    <label for="title"><strong>Judul Agenda</strong> <span class="text-danger">*</span></label>
                    <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title') }}" placeholder="Judul Agenda" /> 
                    @error('title')
                ...
                <div class="form-group ">
                    ...
                    <div class="input-group"  data-target-input="nearest">
                        <input type="text"  name="event_date" id="event_date" class="form-control datetimepicker-input @error('event_date') is-invalid @enderror" data-toggle="datetimepicker" data-target="#event_date" value="{{ old('event_date') }}"  placeholder="Tanggal Pelaksanaan"/>
                ...
                <div class="form-group ">
                    ...
                    <input type="text" name="every" id="title" class="form-control @error('every') is-invalid @enderror" value="{{ old('every') }}" placeholder="Setiap" /> 
                    @error('every')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group ">
                    ...
                    <textarea name="content" id="message" class="form-control  texteditor  @error('content') is-invalid @enderror " rows=" 5 " placeholder="" style="visibility: hidden; display: none;"> {{ old('content') }} </textarea>
                    @error('content')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
